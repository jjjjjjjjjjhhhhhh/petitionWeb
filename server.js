...
app.get("/ansed/:id", (req, res) => {
  res.render("ansed", { login_state: login_states });
  db.collection("petitions").findOne(
    { _id: parseInt(req.params.id) },
    (err, result) => {
      if (result.대답 == true) {
        res.render("ansed", { data: result, login_state: login_states });
      } else {
        redirect("/");
      }
    }
    ...
    db.collection("petitions").insertOne(
      {
        ...
        status: "start",
        청원기간: currDate + "~" + changeDate(year, month, date),
  ...
  db.collection("petitions")
    ...
    .toArray(function (err, result) {
      db.collection("counter").findOne({ name: "postNum" }, (err, rez) => {
        ...
        rep.render("petitionlist", {
          login_state: login_states,
          lengthpetition: rez.totalPost,
          array: result,
        });
      });
    });
